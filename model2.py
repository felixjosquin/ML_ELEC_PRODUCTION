...
hidden_layer_rnn = 48
n_record_rnn = 4  # 1 2 3 4 6 8 12 16 24 48
#############################
...
class Mod(nn.Module):
    ...
    def forward(self, x):
        # x = 1 * L * NB_CHANNELS
        x_periodic = x[:, :, indice_periodic]
        x_no_periodic = x[:, :, indice_no_periodic]

        # Periodic energy type
        x_periodic = torch.transpose(x_periodic, 1, 2)
        # x = 1  * 3 * L
        ...
        x_no_periodic = x_no_periodic[
            :, (nb_days_look_before - 1) * NB_RECORD_FOR_DAY :, :
        ]
        x_no_periodic = x_no_periodic.view(-1, n_record_rnn * len(indice_no_periodic))
